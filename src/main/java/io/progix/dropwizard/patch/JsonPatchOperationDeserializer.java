...
public class JsonPatchOperationDeserializer extends JsonDeserializer<JsonPatchOperation> {
    ...
    /**
     * This method is responsible for deserialization of a {@link JsonPatchOperation}
     * <p/>
     * The value in a patch operation is mapped to a TreeMap and can be later converted to a class of choice using
     * {@link JsonPatchValue}
     *
     * @see JsonDeserializer
     */
    @Override
    public JsonPatchOperation deserialize(JsonParser jp,
            DeserializationContext ctxt) throws IOException, JsonProcessingException {
        untypedObjectDeserializer.resolve(ctxt);

        JsonNode node = jp.getCodec().readTree(jp);

        JsonPatchOperationType operation = JsonPatchOperationType.valueOf(node.get("op").asText().toUpperCase());
